...
function stripBadGuidance(text: string): string {
  return text
    .split("\n")
    .filter((line) => !BAD_PI_DOC_GUIDANCE.some((pattern) => pattern.test(line)))
    .join("\n");
}

function stripSystem(system: unknown): unknown {
  if (typeof system === "string") {
    return stripBadGuidance(system);
  }
...
export default function meridianSystemPromptFilter(pi: {
  on: (eventName: "before_provider_request", handler: (event: { payload: unknown }) => unknown) => void;
}) {
