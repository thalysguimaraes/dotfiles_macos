...
/** Return web-tools settings with env overrides for account-specific values. */
export function getWebToolsSettings(): WebToolsSettings {
	return {
		...
		search: {
			...
			endpoint: mustParsePublicHttpUrl(process.env.EXA_MCP_URL || DEFAULTS.searchEndpoint),
...
function mustParsePublicHttpUrl(input: string): PublicHttpUrl {
	...
	if (parsed._tag === "err") {
		throw new Error("Invalid web-tools search endpoint");
	}
	return parsed.value;
}
