...
function defaultConfigPath(): string {
  return process.env.PI_CLOAK_CONFIG || join(defaultAgentDir(), "cloak.json");
}
...
function compileRule(rule: CloakRuleConfig): CompiledCloakRule {
  return {
    fileRegexes: toArray(rule.filePattern).map(globToRegExp),
    patterns: toArray(rule.cloakPattern).map((pattern) => compilePattern(pattern, rule.replace)),
  };
}

export function loadState(configPath: string = defaultConfigPath()): RuntimeState {
  try {
...
export default function piCloak(pi: {
  on: (
    eventName: "session_start" | "tool_result",
    handler: (event: any, ctx: any) => unknown,
  ) => void;
  registerCommand: (
    name: string,
    command: { description: string; handler: (_args: string, ctx: any) => Promise<void> },
  ) => void;
}) {
  let state = loadState();
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    if (!rawPath || !Array.isArray(event.content)) return undefined;
